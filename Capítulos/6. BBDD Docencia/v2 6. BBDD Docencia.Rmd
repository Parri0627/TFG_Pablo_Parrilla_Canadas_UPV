...
cursa=dataset_base %>%
  ...
  ungroup() %>%
  select(cod_cursa, dni_hash, tit_hash, caca) %>%
  distinct()
...
```{r}
# Rendimientos
rendimientos=dataset_base %>%
  group_by(dni_hash, tit_hash, caca) %>%
  mutate(cod_cursa = paste0("cod_cursa_", cur_group_id())) %>%
  ungroup() %>%
  select(
    cod_cursa,
    rendimiento_cuat_a, rendimiento_total,
    rend_total_ultimo, rend_total_penultimo, rend_total_antepenultimo
  )
...
write.csv(rendimientos, "../../Datos/Capítulos/Base/rendimiento.csv", row.names = FALSE)
...
write.csv(actividad_digital_estudiante, "../../Datos/Capítulos/Base/actividad_digital_estudiante.csv", row.names = FALSE)
write.csv(actividad_digital_asignatura, "../../Datos/Capítulos/Base/actividad_digital_asignatura.csv", row.names = FALSE)
