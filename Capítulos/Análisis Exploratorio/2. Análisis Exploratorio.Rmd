...
# Función que crea un gráfico para un grupo de variables
...
```{r}
a=skim(academicas)
write.csv(a[c(7:43),c(1:4,13:19)],"skim_academicas.csv")

```


Mirar año inicio de estudios

```{r}

# Lista de variables a representar
vars_academicas <- c(
...
# Crear gráfico con etiquetas de conteo
ggplot(df_long, aes(x = Valor, y = fct_reorder(Variable, -Valor, .fun = median, na.rm = TRUE))) +
  geom_boxplot(fill = "skyblue", outlier.size = 0.7) +
...
crear_plot_binarias <- function(vars) {
  ...
  ggplot(df, aes(x = Variable, y = Proporcion, fill = as.factor(Valor))) +
    ...
    scale_fill_manual(values = c("0" = "#2a9d8f", "1" = "#e76f51")) +  # Ejemplo de colores
...
# Convertir a factor ordenado para controlar el orden de las barras
...
variables_asg <- function(df, variable, titulo) {
  ...
  # Quitar valores cero o negativos para el log
  df_filtrado <- df[df[[variable]] > 0, ]

  # Gráfico 1: Boxplot con escala lineal
...
# 1. Selección de variables numéricas y categóricas
num_vars <- sociodemografia %>% select(where(is.numeric))
cat_vars <- sociodemografia %>% select(where(is.factor) | where(is.character)) %>% 
  select(-dni_hash)  # Eliminar identificadores

# 2. One-hot encoding para categóricas
cat_dummies <- dummy_cols(cat_vars, remove_first_dummy = TRUE, remove_selected_columns = TRUE)

# 3. Combinar numéricas + dummies
datos_pca <- bind_cols(num_vars, cat_dummies)

# 4. Eliminar columnas constantes (varianza 0)
datos_pca <- datos_pca[, sapply(datos_pca, function(x) var(x, na.rm = TRUE) > 0)]

# 5. Imputar NA con mediana
preprocess <- preProcess(datos_pca, method = "medianImpute")
datos_imputados <- predict(preprocess, newdata = datos_pca)

# 6. PCA
pca_resultado <- prcomp(datos_imputados, scale. = TRUE)

# 7. Ver resumen y contribuciones
...
fviz_pca_var(pca_resultado,
             ...
             gradient.cols = c("red", "yellow", "green"),  # Paleta de colores
...
cat_vars <- academicas %>% select(where(is.factor) | where(is.character)) %>% 
  select(-dni_hash)  # Eliminar identificadores
...
# 7. Ver resumen y contribuciones
summary(pca_resultado)
fviz_eig(pca_resultado, addlabels = TRUE, main="PCA Poliformat")
contrib <- factoextra::get_pca_var(pca_resultado)$contrib

# 2. Filtrar variables con más del 10% en al menos una de las dos primeras dimensiones
...
# 3. Visualizar solo esas variables
fviz_pca_var(pca_resultado,
...
cat_vars <- ambas %>% select(where(is.factor) | where(is.character)) %>% 
  select(-dni_hash)  # Eliminar identificadores
...
num_vars <- completo %>% select(where(is.numeric))
cat_vars <- completo %>% select(where(is.factor) | where(is.character)) %>% 
  select(-dni_hash)  # Eliminar identificadores
