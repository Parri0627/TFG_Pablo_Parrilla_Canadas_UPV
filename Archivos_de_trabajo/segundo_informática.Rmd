...
abandono_inf=abandono[abandono$tit_hash=="0fecf9247f3d",]
...
Cuánta gente se ha cambiado de carrera?
...
df <- demografía_inf[,c("nacionalitat",
...
# ⚠️ Asegurate de que los niveles estén en el orden correcto

# Paso 4: Crear dummies para variables categóricas nominales
df_dummies <- fastDummies::dummy_cols(df, remove_selected_columns = TRUE)

# Paso 5: Seleccionar solo columnas numéricas para correlación
...
corrplot(cor_mat,
         ...
         tl.cex = 0.7,              # tamaño etiquetas
         tl.col = "black",          # color texto etiquetas
         addCoef.col = NA,          # 🔥 esto quita los numeritos
         number.cex = 0,            # adicional para evitar que salgan
         cl.cex = 0.7,              # tamaño de la leyenda lateral
...
# Asegurarse de que abandono es numérica binaria (0/1)
demografía_inf$abandono <- as.numeric(as.factor(demografía_inf$abandono)) - 1

# Filtrar solo variables seleccionadas + abandono
df_sel <- demografía_inf[, c("abandono", vars_usar)]
...
# Añadir sexe y nacionalitat a las numéricas manualmente (ya las convertimos)
...
ggplot(resultados, aes(x = reorder(variable, correlacion), y = correlacion, fill = tipo)) +
  ...
  theme_minimal() +
  labs(title = "Correlación de variables con 'abandono'",
       x = "Variable",
